...
class ResponseTile extends StatefulWidget {
  ...
  const ResponseTile({Key? key, required this.answer}) : super(key: key);
...
class ResponseTileState extends State<ResponseTile> {
  ...
  void initState() {
    super.initState();
    // Check if the answer is selected and update the state
    isSelected = Provider.of<GameProvider>(context, listen: false)
        .isSelected(widget.answer['text']);
  }
          ...
          onTap: () {
            if (gameProvider.canSelected) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('YOU MUST ATTEND THIS QUESTION'),
                ),
              );
            }
          },
